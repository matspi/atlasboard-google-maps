...
onRun: function (config, dependencies, jobCallback) {
  ...
  */
  ...
  It is a good idea to cover this with unit tests as well (see test/google-maps-route file)
...
Have a look at test/google-maps-route for an example of how to unit tests this easily by mocking easyRequest calls

*/

var url = "https://maps.googleapis.com/maps/api/directions/json?";

url += "origin=" + encodeURIComponent(config.source);
url += "&destination=" + encodeURIComponent(config.destination);
url += "&mode=driving&departure_time=now&alternatives=false";
url += "&key=" + config.globalAuth['google-apis'].maps;

var retVal = "";

dependencies.easyRequest.HTML(url, function(err, json) {
  // dependencies.logger.info(json);
  var results = JSON.parse(json);

  jobCallback(null, { "apikey": config.globalAuth['google-apis'].maps, "route" : results});
});
}
};
